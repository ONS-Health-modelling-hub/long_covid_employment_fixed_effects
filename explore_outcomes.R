...
### derive never infected vs infected without LC vs infected with LC
dat$ever_infected_lc <- "0_never_infected"
dat$ever_infected_lc[dat$ever_infected==1 & dat$ever_lc_any==0] <- "1_ever_infected_without_lc"
dat$ever_infected_lc[dat$ever_infected==1 & dat$ever_lc_any==1] <- "2_ever_infected_with_lc"
...
### they were ever infected, with or without Long Covid
ever_inactive_by_ever_lc <- sqldf("
  ...
  from(
    select participant_id, ever_infected_lc, max(notlooking) as notlooking
    from dat
    group by participant_id, ever_infected_lc
  )
  group by ever_infected_lc
")
...
ever_working_by_ever_lc <- sqldf("
  ...
  from(
    select participant_id, ever_infected_lc, max(working) as working
    from dat
    group by participant_id, ever_infected_lc
  )
  group by ever_infected_lc
...
### find percentage of who participants who were ever retired according to whether
### they were ever infected, with or without Long Covid
ever_retired_by_ever_lc <- sqldf("
  select
    ever_infected_lc,
    count(*) as n_participants,
  ...
  from(
    select participant_id, ever_infected_lc, max(retired) as retired
    from dat
    group by participant_id, ever_infected_lc
  )
  group by ever_infected_lc
...
write.csv(ever_retired_by_ever_lc,
          file=paste0(out_dir, "\\ever_retired_by_ever_lc.csv"),
          row.names=FALSE)
...
### find percentage of who participants who were ever unemployed according to whether
### they were ever infected, with or without Long Covid
ever_unemployed_by_ever_lc <- sqldf("
  select
    ever_infected_lc,
    count(*) as n_participants,
  ...
  from(
    select participant_id, ever_infected_lc, max(unemployed) as unemployed
    from dat
    group by participant_id, ever_infected_lc
  )
  group by ever_infected_lc
...
write.csv(ever_unemployed_by_ever_lc,
          file=paste0(out_dir, "\\ever_unemployed_by_ever_lc.csv"),
          row.names=FALSE)
...
### they were ever infected, with or without Long Covid (whilst in work, visits
### from 1 Oct 2021 only)
ever_offwork_by_ever_lc <- sqldf("
  ...
  from(
    select participant_id, ever_infected_lc, max(offwork) as offwork
    from dat
