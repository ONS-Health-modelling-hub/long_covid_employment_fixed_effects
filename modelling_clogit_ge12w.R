modelling.clogit.ge12w <- function(dset, chart_cols, out_dir, outcome,
  ...
  if(run_mod1==TRUE) {
    ...
    or1$grp2 <- factor(or1$grp2,
                       labels=c("Infected 12+ weeks ago",
  ...
  if(run_mod2==TRUE) {
    ...
    or2$time <- NA
    or2$time[grep("12-18w", or2$exposure)] <- 1
    or2$time[grep("18-24w", or2$exposure)] <- 2
    or2$time[grep("24-30w", or2$exposure)] <- 3
    or2$time[grep("30-40w", or2$exposure)] <- 4
    or2$time[grep("40-52w", or2$exposure)] <- 5
    or2$time[grep("ge52w", or2$exposure)] <- 6
    
    ### derive variable for infection/LC status
    or2$status <- NA
    or2$status[grep("nolc", or2$exposure)] <- 1
    or2$status[grep("currlc", or2$exposure)] <- 2
    or2$status[grep("prevlc", or2$exposure)] <- 3
    ...
    or2$exposure <- factor(or2$exposure,
                           labels=c("12 to <18 weeks, previously infected", "18 to <24 weeks, previously infected",
    ...
    or2_plot_facet <- ggplot(or2, aes(x=or, y=time, group=status, colour=status, label=label)) +
      ...
      scale_colour_manual(values=chart_cols[3:1]) +
      xlab("Odds ratio (log scale)") +
      theme(
        axis.title.x=element_text(size=9, colour="black", face="bold"),
        axis.text.x = element_text(size=8, colour="black", face="plain"),
