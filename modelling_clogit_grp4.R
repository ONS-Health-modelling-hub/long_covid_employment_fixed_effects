modelling.clogit.grp4 <- function(dset, chart_cols, out_dir, outcome,
  ...
  if(run_mod1==TRUE) {
    ...
    or1_unadj <- mod1_unadj$coeff[grep("grp4", rownames(mod1_unadj$coeff)),]
    or1_unadj$model <- "1_unadjusted"
    or1_unadj$grp4 <- rownames(or1_unadj)
    
    ### coeffs for adjusted model
    or1_adj <- mod1_adj$coeff[grep("grp4", rownames(mod1_adj$coeff)),]
    or1_adj$model <- "2_adjusted"
    or1_adj$grp4 <- rownames(or1_adj)
    ...
    or1$grp4 <- factor(or1$grp4,
                       labels=c("Current Long Covid", "Previous Long Covid"))
    
    or1$grp4 <- factor(or1$grp4, levels=levels(or1$grp4)[nlevels(or1$grp4):1])
    ...
    or1_plot_facet <- ggplot(or1, aes(x=or, y=grp4, label=label)) +
    ...
    or1_plot_adj <- ggplot(or1[or1$model=="Adjusted",], aes(x=or, y=grp4, label=label)) +
      geom_vline(xintercept=1, linetype="dashed", colour="grey40", size=0.3) +
  ...
  if(run_mod2==TRUE) {
    ...
    ### coeffs for unadjusted model
    or2_unadj <- mod2_unadj$coeff[grep("exposure2", rownames(mod2_unadj$coeff)),]
    or2_unadj$model <- "1_unadjusted"
    or2_unadj$exposure2 <- rownames(or2_unadj)
    
    ### coeffs for adjusted model
    or2_adj <- mod2_adj$coeff[grep("exposure2", rownames(mod2_adj$coeff)),]
    or2_adj$model <- "2_adjusted"
    or2_adj$exposure2 <- rownames(or2_adj)
    ...
    or2$time <- NA
    or2$time[grep("12-18w", or2$exposure2)] <- 1
    or2$time[grep("18-24w", or2$exposure2)] <- 2
    or2$time[grep("24-30w", or2$exposure2)] <- 3
    or2$time[grep("30-40w", or2$exposure2)] <- 4
    or2$time[grep("40-52w", or2$exposure2)] <- 5
    or2$time[grep("ge52w", or2$exposure2)] <- 6
    
    ### derive variable for infection/LC status
    or2$status <- NA
    or2$status[grep("nolc", or2$exposure2)] <- 1
    or2$status[grep("currlc", or2$exposure2)] <- 2
    or2$status[grep("prevlc", or2$exposure2)] <- 3
    ...
    or2$time <- factor(or2$time,
                       labels=c("12 to <18 weeks", "18 to <24 weeks",
    ...
    or2$exposure2 <- factor(or2$exposure2,
                            labels=c("12 to <18 weeks, current Long Covid", "18 to <24 weeks, current Long Covid",
                                     "24 to <30 weeks, current Long Covid", "30 to <40 weeks, current Long Covid",
                                     "40 to <52 weeks, current Long Covid", "52+ weeks, current Long Covid",
                                     ...
                                     "40 to <52 weeks, previous Long Covid", "52+ weeks, previous Long Covid"))
    
    or2$exposure2 <- factor(or2$exposure2, levels=levels(or2$exposure2)[nlevels(or2$exposure2):1])
    ...
    or2_plot_facet <- ggplot(or2, aes(x=or, y=time, group=status, colour=status, label=label)) +
      ...
      scale_colour_manual(values=chart_cols[2:1]) +
      xlab("Odds ratio (log scale)") +
      theme(
        axis.title.x=element_text(size=9, colour="black", face="bold"),
        axis.text.x = element_text(size=8, colour="black", face="plain"),
