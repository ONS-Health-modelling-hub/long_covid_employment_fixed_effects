modelling.logistic <- function(dset, chart_cols, out_dir, outcome,
  ...
  if(run_mod1==TRUE) {
    ...
    mod1_unadj <- fit.logistic.model(outcome=outcome,
                                     exposures="grp2",
                                     covariates=NULL,
                                     dataset=dset)
    
    ### fit adjusted model
    mod1_adj <- fit.logistic.model(outcome=outcome,
                                   exposures="grp2",
                                   covariates=covs,
                                   dataset=dset,
                                   run.lr=FALSE)
    ...
    mod1_adj_baseline <- fit.logistic.model(outcome=outcome,
                                            exposures="1",
                                            covariates=covs,
                                            dataset=dset,
                                            run.lr=FALSE)
    ...
    ### calculate ORs
    or1$or <- exp(or1[,1])
    or1$lcl <- exp(or1[,1] - 1.96*or1[,2])
    or1$ucl <- exp(or1[,1] + 1.96*or1[,2])
    ...
    ### write out ORs
    write.csv(or1[,5:9], file=paste0(out_dir, "\\or_mod1.csv"), row.names=FALSE)
  ...
  if(run_mod2==TRUE) {
    ...
    ### fit unadjusted model
    mod2_unadj <- fit.logistic.model(outcome=outcome,
                                     exposures="exposure",
                                     covariates=NULL,
                                     dataset=dset)
    
    ### fit adjusted model
    mod2_adj <- fit.logistic.model(outcome=outcome,
                                   exposures="exposure",
                                   covariates=covs,
                                   dataset=dset,
                                   run.lr=FALSE)
    ...
    ### fit adjusted model - baseline
    mod2_adj_baseline <- fit.logistic.model(outcome=outcome,
                                            exposures="1",
                                            covariates=covs,
                                            dataset=dset,
                                            run.lr=FALSE)
    ...
    ### calculate ORs
    or2$or <- exp(or2[,1])
    or2$lcl <- exp(or2[,1] - 1.96*or2[,2])
    or2$ucl <- exp(or2[,1] + 1.96*or2[,2])
    ...
    or2$time[grep("40-52w", or2$exposure)] <- 6
    or2$time[grep("ge52w", or2$exposure)] <- 7
    ...
    or2$exposure <- factor(or2$exposure,
                           labels=c("<12 weeks",
                                    ...
                                    "40 to <52 weeks, previously infected", "52+ weeks, previously infected",
    ...
    ### write out ORs
    write.csv(or2[,5:9], file=paste0(out_dir, "\\or_mod2.csv"), row.names=FALSE)
